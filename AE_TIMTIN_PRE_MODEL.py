...
# def update_weights_decoder_test(TxNet, Normalize, RxNet, K, X, Y, lr_rate):

#     Loss = []
#     for k in range(K):
#         with tf.GradientTape() as tape:
#             # RxSig = TxNet[k].transmit(X[k])
#             RxSig = TxNet[k].transmit(X[k])
#             RxSig = RxNet[k].receive(RxSig)
#             l = tf.keras.losses.mse(Y[k], RxSig)
#             Loss.append(l)
#         Gradients = tape.gradient(l, RxNet[k].trainable_variables)
#         optimizer = tf.keras.optimizers.Adam(lr_rate)
#         optimizer.apply_gradients((zip(Gradients, RxNet[k].trainable_variables)))
